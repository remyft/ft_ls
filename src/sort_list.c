...
/*   Updated: 2018/09/29 23:59:42 by rfontain         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		check_sort(t_lst *list, t_fg *g_fg, int sort)
{
	...
	{
		if ((((*g_fg) & REVERSE &&
			list->itime > list->next->itime) || (!((*g_fg) & REVERSE) &&
...
{
	if (!(*g_fg & DATE_SORT) && !(*g_fg & SIZE_SORT) && check_sort(list, g_fg, 0))
		return (1);
	else if (*g_fg & DATE_SORT && !(*g_fg & SIZE_SORT) && check_sort(list, g_fg, 1))
		return (1);
	else if (check_sort(list, g_fg, 2))
		return (1);
	return (0);
}
